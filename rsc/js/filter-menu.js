...
class FilterMenu extends HTMLElement {
  ...
  _render() {
    this.shadowRoot.innerHTML = `
      <style>
        ...
        *, *::before, *::after {
        ...
        .filter-dropdown-menu {
          ...
          color: var(--md-sys-color-on-surface, #1c1b1f);
          box-shadow: 0 12px 28px rgba(31, 22, 59, 0.16);
        }
        ...
        .tech-filter-header {
          padding: 12px 16px;
          font-weight: 650;
          border-bottom: 1px solid rgba(103, 80, 164, 0.14);
        ...
        .filter-menu-item:hover {
          background: rgba(103, 80, 164, 0.085);
        }
        ...
        .tech-filter-card {
          ...
          background: var(--md-sys-color-surface, #ffffff);
          color: var(--md-sys-color-on-surface, #1c1b1f);
          box-shadow: 0 12px 28px rgba(31, 22, 59, 0.18);
        }

        .filter-tech-trigger:hover .tech-filter-card,
        .filter-tech-trigger:focus-within .tech-filter-card,
        ...
        .tech-search-input::placeholder {
          color: var(--md-sys-color-on-surface-variant, #6f6878);
        }
        ...
        .custom-filter-chip:hover {
          background: rgba(103, 80, 164, 0.13);
        }
        ...
        :host-context(body.dark-mode) #filterToggleBtn {
          color: var(--md-sys-color-on-surface, #e6e0e9);
        }

        :host-context(body.dark-mode) .filter-dropdown-menu,
        :host-context(body.dark-mode) .tech-filter-card {
        ...
        :host-context(body.dark-mode) .filter-menu-header,
        :host-context(body.dark-mode) .tech-filter-header {
          background: rgba(208, 188, 255, 0.10);
          color: rgba(245, 241, 255, 0.94);
          border-bottom-color: rgba(208, 188, 255, 0.16);
        }

        :host-context(body.dark-mode) .filter-menu-item:hover,
        :host-context(body.dark-mode) #filterToggleBtn:hover,
        :host-context(body.dark-mode) #filterToggleBtn:focus-visible {
          background: rgba(208, 188, 255, 0.12);
          border-color: rgba(208, 188, 255, 0.20);
        }

        :host-context(body.dark-mode) .tech-search-input {
          background: rgba(255, 255, 255, 0.065);
          color: rgba(245, 241, 255, 0.94);
          border-color: rgba(208, 188, 255, 0.22);
        }

        :host-context(body.dark-mode) .tech-search-input::placeholder,
        :host-context(body.dark-mode) .tech-filter-title,
        :host-context(body.dark-mode) .tech-empty-state {
          color: rgba(245, 241, 255, 0.66);
        }

        :host-context(body.dark-mode) .custom-filter-chip {
          background: rgba(208, 188, 255, 0.10);
          color: rgba(245, 241, 255, 0.88);
          border-color: rgba(208, 188, 255, 0.18);
        }

        :host-context(body.dark-mode) .custom-filter-chip:hover {
          background: rgba(208, 188, 255, 0.16);
        }

        :host-context(body.dark-mode) .custom-filter-chip.selected {
      ...
      <button id="filterToggleBtn" type="button" aria-label="Filter Options" aria-expanded="false">
      ...
      <div class="filter-dropdown-menu" role="menu" aria-label="Project filter options">
        ...
        <label class="filter-menu-item">
          <input type="checkbox" id="filterMermaid" />
          <span>Has Mermaid</span>
        </label>
        ...
        <div class="filter-menu-item filter-tech-trigger" tabindex="0">
          ...
          <div class="tech-filter-card" aria-label="Technology filters">
            <div class="tech-filter-header">
              <label>
                <span class="tech-filter-title">Technologies</span>
                <input id="techSearch" class="tech-search-input" type="search" placeholder="Search technologies" autocomplete="off" />
              </label>
            </div>
...
function injectResumeModalPolish() {
  ...
  style.textContent = `
    #pdfModal .modal-content {
      ...
      border: 1px solid rgba(103, 80, 164, 0.18) !important;
      box-shadow: 0 20px 48px rgba(31, 22, 59, 0.22) !important;
    }
    ...
    #pdfModal .close-button:hover,
    #pdfModal .close-button:focus-visible {
    ...
    body.dark-mode #pdfModal .modal-content {
      background: var(--md-sys-color-surface-container, #1d1b20) !important;
      color: var(--md-sys-color-on-surface, #e6e0e9) !important;
      border-color: rgba(208, 188, 255, 0.18) !important;
      box-shadow: 0 22px 54px rgba(0, 0, 0, 0.46) !important;
    }

    body.dark-mode #pdfModal .close-button {
      background: rgba(208, 188, 255, 0.10) !important;
      color: rgba(245, 241, 255, 0.94) !important;
      border-color: rgba(208, 188, 255, 0.20) !important;
    }

    body.dark-mode #pdfModal .close-button:hover,
    body.dark-mode #pdfModal .close-button:focus-visible {
      background: rgba(208, 188, 255, 0.16) !important;
      border-color: rgba(208, 188, 255, 0.32) !important;
    }
  `;
...
function repairProjectsPerPageSelect() {
  ...
  if (!select) {
    container.innerHTML = `
      ...
      <select id="perPageSelect"></select>
    `;
  ...
  const options = [...new Set([...PER_PAGE_OPTIONS, currentValue])]
    .filter(Number.isFinite)
    .sort((a, b) => a - b);

  const previousValue = String(currentValue);
  select.innerHTML = options.map((option) => `<option value="${option}">${option}</option>`).join('');
  select.value = options.includes(currentValue) ? previousValue : String(options[0]);
...
function installProjectsPerPageRepair() {
  const runRepair = () => window.requestAnimationFrame(repairProjectsPerPageSelect);
  runRepair();
  setTimeout(runRepair, 250);
  setTimeout(runRepair, 750);

  const container = document.getElementById('perPageContainer');
  if (container && !container.dataset.perPageObserverAttached) {
    container.dataset.perPageObserverAttached = 'true';
    const observer = new MutationObserver(runRepair);
...
if (!customElements.get(FILTER_MENU_TAG)) {
  customElements.define(FILTER_MENU_TAG, FilterMenu);
}
